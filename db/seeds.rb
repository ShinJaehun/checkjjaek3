...
# =========================
# sample groups for OCI/demo
# =========================

admin = User.find_by!(email: "a@a")
user_b = User.find_by!(email: "b@b")
user_c = User.find_by!(email: "c@c")
user_d = User.find_by!(email: "d@d")

sample_groups = [
  {
    name: "책짹 운영팀",
    description: "배포 직후 동아리 기능 점검용 운영 그룹",
    manager: admin,
    members: [user_b, user_c]
  },
  {
    name: "최강로떼",
    description: "롯데자이언츠를 응원하는 동아리",
    manager: user_b,
    members: [admin, user_d]
  },
  {
    name: "제주 여행 기록단",
    description: "여행책과 여행 기록을 함께 나누는 동아리",
    manager: user_d,
    members: [admin, user_c]
  }
]

sample_groups.each do |attrs|
  group = Group.find_or_initialize_by(name: attrs[:name])
  ...
  manager_membership = UserGroup.find_or_initialize_by(
    user_id: manager.id,
    group_id: group.id
  )
  manager_membership.state = "active"
  manager_membership.save!

  manager.add_role(:group_manager, group) unless manager.has_role?(:group_manager, group)
  manager.add_role(:group_member, group) unless manager.has_role?(:group_member, group)

  attrs[:members].each do |member|
    membership = UserGroup.find_or_initialize_by(
      user_id: member.id,
      group_id: group.id
    )
    ...
    member.add_role(:group_member, group) unless member.has_role?(:group_member, group)
  end
end
